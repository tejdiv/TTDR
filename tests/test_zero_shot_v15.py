"""Test octo-base-1.5 with timestep_pad_mask key (matching its example_batch)."""
...
def run_test(pad_mask_key="pad_mask", num_eps=20):
    """Run zero-shot eval with octo-base-1.5, using specified obs key for pad mask."""
    print(f"\n--- Testing with obs key: '{pad_mask_key}' ---")
    ...
    print("example_batch obs keys:", sorted(model.example_batch["observation"].keys()))
    ...
    for ep in range(num_eps):
        ...
        while not done:
            ...
            # Key difference: use the specified key name
            input_obs = {"image_primary": images, pad_mask_key: pad_mask}
    ...
    print(f"\nocto-base-1.5 ({pad_mask_key}): {successes}/{num_eps} = {successes/num_eps:.3f}")
    env.close()
    return successes / num_eps


if __name__ == "__main__":
    # Test with timestep_pad_mask (matches octo-base-1.5's example_batch)
    run_test(pad_mask_key="timestep_pad_mask", num_eps=20)
